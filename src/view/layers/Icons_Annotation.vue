    ...
    <Col span="4">
          <Card class="demo-split ">
            ...
            <div class="treeCompany">
              <v-tree ref='tree' @node-click="clickNode" :data='treeData1' :radio="true" :multiple="false" :halfcheck='true' />
            </div>
          </Card>
    </Col>
    <Col span="16" style="padding-left:10px; padding-right:10px;">
    <Card class="layer_main">
      <p slot="title"><span @click="isLayer=true" :class="isLayer?'layer_active':'layer_noActive'">分层图</span> <span @click="isLayer=false" style="padding-left:10px;" :class="!isLayer?'layer_active':'layer_noActive'">设备列表</span>
      ...
      <div v-else>
        ...
        <Table stripe :height="getTableheight" class="tableInfo" :columns="columns1" :data="data.dataContent"></Table>
      </div>
      ...
      <Collapse v-model="value1" simple accordion @on-change="editChange">
          ...
          <div slot="content">
            ...
            <p class="editP">
              <Select style="width:100%" v-model="searchItem.type">
                       <Option v-for="item in pointValueTypeList" :value="item.value"  :key="item.value">{{ item.label }}</Option>
              </Select>
            </p>
            <p class="editP">
            ...
            <div>
              ...
              <p class="editP">
                <Select style="width:100%" v-model="tempSV.deviceId">
                         <Option v-for="item in deviceList" :value="item.value" @click.native="getdevicesMoreParamsforAdd(item)"  :key="item.value">{{ item.label }}</Option>
                </Select>
              </p>
              <p>设备点位-X</p>
              <p class="editP">
              ...
              <p class="editP">
                <Select style="width:100%" v-model="tempSV.deviceType">
                         <Option v-for="item in pointValueTypeList" :value="item.value" @click.native="getdevicesTypeMoreParamsforAdd(item)" :key="item.value">{{ item.label }}</Option>
                </Select>
              </p>
              <p class="editP">
        ...
        <Panel name="3">
          ...
          <div slot="content">
            ...
            <p class="editP">
              <Select style="width:100%">
                         <Option v-for="item in pointValueTypeList" :value="item.value" @click.native="getdevicesTypeMoreParams(item)" :key="item.value">{{ item.label }}</Option>
                     </Select>
            </p>
            <p class="editP">
...
export default {
  ...
  methods: {
    ...
    getdevicesTypeMoreParams(option) {
      ...
      this.pointValue.pointImage = option.uri .indexOf( window.g.SERVICE_PATH)!= -1 ?option.uri: window.g.SERVICE_PATH+option.uri;
    },
    ...
    save() //保存点信息
    {
      ...
      this.setMarkerAction({
        ...
        avatar:this.addpointValue.pointImage.indexOf(window.g.SERVICE_PATH) !=-1?this.addpointValue.pointImage.substr(window.g.SERVICE_PATH.length):this.addpointValue.pointImage,
    ...
    update() {
      ...
      this.updateMarkerAction({
        ...
        y: this.pointValue.y,
        type_id: 2,
        avatar: this.addpointValue.pointImage.substr(window.g.SERVICE_PATH.length),
    ...
    showAll() {
      ...
      this.data.dataContent = this.tempData.filter(item => {
        if (that.searchItem.name != "" && that.searchItem.type != "") {
          if (item.name.indexOf(that.searchItem.name) != -1 && item.type == that.searchItem.type) {
            return true;
          } else {
            return false;
          }
        } else if (that.searchItem.name && that.searchItem.name != "" || that.searchItem.type && that.searchItem.type != "") {
          var temp = false;
          if (that.searchItem.name != "" && item.name.indexOf(that.searchItem.name) != -1) {
            temp = true;
          }
